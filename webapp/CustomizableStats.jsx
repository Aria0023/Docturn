...
function CustomizableStats({ statKey, stats }) {
  const a = useActions();
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
        {editing && (
          <span style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 12, color: "var(--muted-foreground)" }}>
            <Icon name="grip-vertical" size={13} />Drag to reorder · remove or add stats
          </span>
        )}
        <div style={{ marginLeft: "auto", display: "flex", gap: 8 }}>
          {editing && hiddenStats.length > 0 && (
            <div style={{ position: "relative" }}>
              <Button size="sm" variant="outline" icon="plus" onClick={() => setAdding((v) => !v)}>Add stat ({hiddenStats.length})</Button>
              {adding && (
                <React.Fragment>
          ...
          <Button size="sm" variant={editing ? "default" : "ghost"} icon={editing ? "check" : "sliders-horizontal"} onClick={() => { setEditing((v) => !v); setAdding(false); }}>
